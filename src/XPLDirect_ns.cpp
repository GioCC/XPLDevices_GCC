...
namespace XPLDirect
{
    ...
    static Stream *streamPtr;
    static char   *_deviceName;
    static char    _receiveBuffer[XPLMAX_PACKETSIZE];
    static int     _receiveBufferBytesReceived;
    static char    _sendBuffer[XPLMAX_PACKETSIZE];
    static int     _connectionStatus;
    static int     _dataRefsCount;
    static int     _commandsCount;
    static byte    _allDataRefsRegistered; // becomes true if all datarefs have been registered
    static byte    _datarefsUpdatedFlag;   // becomes true if any datarefs have been updated from xplane since last call to datarefsUpdated()

    static struct _dataRefStructure*_dataRefs[XPLDIRECT_MAXDATAREFS_ARDUINO];
    static struct _commandStructure*_commands[XPLDIRECT_MAXCOMMANDS_ARDUINO];

    // Private function prototypes

    static void    _processSerial();
    static void    _processPacket();
    static void    _sendPacketInt(int command, int handle, long int value); // for ints
    static void    _sendPacketFloat(int command, int handle, float value);  // for floats
    static void    _sendPacketVoid(int command, int handle);                // just a command with a handle
    static void    _sendPacketString(int command, char *str);               // for a string
    static void    _transmitPacket();
    static void    _sendname();
    static void    _sendVersion();
    static int     _getHandleFromFrame();
    static int     _getPayloadFromFrame(long int *);
    static int     _getPayloadFromFrame(float *);
    static int     _getPayloadFromFrame(char *);
    ...
    static void _sendname()
    {
        if (_deviceName != NULL) { _sendPacketString(XPLRESPONSE_NAME, _deviceName); }
    }

    static void _sendVersion()
    {
        if (_deviceName != NULL) { _sendPacketInt(XPLRESPONSE_VERSION, XPLDIRECT_ID, XPLDIRECT_VERSION); }
    }
    ...
    static void _processPacket()
    {
        ...
        case XPLCMD_SENDREQUEST: {
            ...
            {
                if (_dataRefs[i]->dataRefHandle == -1) { // some boards cant do sprintf with floats so this is a workaround
    ...
    static void _transmitPacket(void)
    {
    ...
    static int _getHandleFromFrame() // Assuming receive buffer is holding a good frame
    {
        char holdChar;
        int  handleRet;
    ...
    static int _getPayloadFromFrame(long int *value) // Assuming receive buffer is holding a good frame
    {
        char holdChar;
    ...
    static int _getPayloadFromFrame(float *value) // Assuming receive buffer is holding a good frame
    {
        char holdChar;
        holdChar           = _receiveBuffer[15];
        _receiveBuffer[15] = 0;
    ...
    static int _getPayloadFromFrame(char *value) // Assuming receive buffer is holding a good frame
    {
    ...
    {
        if (commandHandle < 0 || commandHandle >= _commandsCount) { // invalid handle
            return -1;
        }
        if (!_commands[commandHandle]) { // inactive command
            return -1;
        }
#if XPL_DEBUG
