...
void DigitalIn_::setMuxChannel(uint8_t ch)
{
    ...
    cli();
    preg = portOutputRegister(_s3port);
    (ch & 0x08) ? (*preg |= _s3mask) : (*preg &= ~_s3mask);
    preg = portOutputRegister(_s2port);
    (ch & 0x04) ? (*preg |= _s2mask) : (*preg &= ~_s2mask);
    preg = portOutputRegister(_s3port);
    (ch & 0x02) ? (*preg |= _s1mask) : (*preg &= ~_s1mask);
    preg = portOutputRegister(_s0port);
    (ch & 0x01) ? (*preg |= _s0mask) : (*preg &= ~_s0mask);
    SREG = oldSREG;
