...
Switch::callback Switch::onChange = nullptr;

void Switch::setCallbacks(callback cb_onChange)
{ 
...
Switch2::callback Switch2::onChange = nullptr;

void Switch2::setCallbacks(callback cb_onChange)
{ 
    onChange = cb_onChange; 
}
...
int Switch2::getCommand()
{
    ...
    if (_cmdOn2 == -1) {
        if (_state == switchOn1) {
            res = _cmdOn1;
        } else if (_state == switchOff && _lastState == switchOn1) {
            res = _cmdOff;
